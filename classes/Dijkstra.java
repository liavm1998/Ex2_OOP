...
public final class Dijkstra {
    Map<Object, Double> resultSize;
    Map<Object, List<Object>> resultPath;
    Map<MyNode, Double> MyResult;
    public Dijkstra(MyGraph g,MyNode v)
    {
        this.MyResult =  shortestPaths(g,v,0);

    }

    public Map<Object, Double> getResultSize()
    {
        return resultSize;
    }
     ...
     * +infinity.
     *
     * @param graph The graph upon which to run classes.Dijkstra's algorithm.
    ...
    public static Map<MyNode, Double> shortestPaths(MyGraph graph, MyNode source,int ppppp)
    {
        /* Create a Fibonacci heap storing the distances of unvisited nodes
         * from the source node.
         */
        FibonacciHeap<MyNode> pq = new FibonacciHeap<MyNode>();

        /* The Fibonacci heap uses an internal representation that hands back
         * Entry objects for every stored element.  This map associates each
         * node in the graph with its corresponding Entry.
         */
        Map<MyNode, FibonacciHeap.Entry<MyNode>> entries = new HashMap<MyNode, FibonacciHeap.Entry<MyNode>>();

        /* Maintain a map from nodes to their distances.  Whenever we expand a
         * node for the first time, we'll put it in here.
         */
        Map<MyNode, Double> result = new HashMap<MyNode, Double>();

        /* Add each node to the Fibonacci heap at distance +infinity since
         * initially all nodes are unreachable.
         */
        for (MyNode node: graph.getV().values())
            entries.put(node, pq.enqueue(node, Double.POSITIVE_INFINITY));
        ...
        while (!pq.isEmpty()) {
             ...
             * have the shortest distance to it.
             */
            FibonacciHeap.Entry<MyNode> curr = pq.dequeueMin();
            ...
            MyNode v= (curr.getValue());
            Iterator<MyEdge> i = v.getOutlist().iterator();
            while(i.hasNext())
            {
                MyEdge arc=i.next();
                /* If we already know the shortest path from the source to
                 * this node, don't add the edge.
                 */
                if (result.containsKey(arc.getDest())) continue;

                /* Compute the cost of the path from the source to this node,
                 * which is the cost of this node plus the cost of this edge.
                 */
                double pathCost = curr.getPriority() + arc.getWeight();

                /* If the length of the best-known path from the source to
                 * this node is longer than this potential path cost, update
                 * the cost of the shortest path.
                 */
                FibonacciHeap.Entry<MyNode> dest = entries.get(graph.getNode((arc.getDest())));
                if (pathCost < dest.getPriority())
                    pq.decreaseKey(dest, pathCost);
            }
        }
